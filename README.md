...
### 4. Create the configuration directory and config file

The service looks for its config at `/etc/prusaconnectcamera/config.json` by
default.  If the file does not exist the service will auto-generate a template
and exit immediately — edit the template, then restart.

```bash
sudo mkdir -p /etc/prusaconnectcamera
sudo .venv/bin/python main.py   # generates /etc/prusaconnectcamera/config.json and exits
sudo nano /etc/prusaconnectcamera/config.json
```
...
```jsonc
{
  "state_dir": "/var/lib/prusaconnectcamera",   // optional, default shown
  "cameras": [
    {
      "name": "USB Camera 1",
      "enabled": true,
...
```bash
sudo chown prusaconnectcamera:prusaconnectcamera /etc/prusaconnectcamera/config.json
sudo chmod 600 /etc/prusaconnectcamera/config.json
```

#### 5. Create the state directory

```bash
sudo mkdir -p /var/lib/prusaconnectcamera
sudo chown prusaconnectcamera:prusaconnectcamera /var/lib/prusaconnectcamera
```
