...
```bash
sudo apt install ffmpeg libcamera-tools wireless-tools
```

`ffmpeg` is required for USB/V4L2 cameras and RTSP snapshot/stream handling.  
`libcamera-tools` (provides `libcamera-still` and `libcamera-vid`) is required for the CSI camera.  
`wireless-tools` (provides `iwgetid`) enables `wifi_ssid` reporting in camera attributes when the Pi is connected via Wi-Fi.

MediaMTX is required for local RTSP streaming. Install it once; the Python service auto-generates MediaMTX config and starts/stops it automatically.

```bash
cd /tmp
curl -fsSL https://github.com/bluenviron/mediamtx/releases/latest/download/mediamtx_linux_arm64.tar.gz -o mediamtx_linux_arm64.tar.gz
tar -xzf mediamtx_linux_arm64.tar.gz
sudo install -m 0755 mediamtx /usr/local/bin/mediamtx
mediamtx --version
```

Use the appropriate MediaMTX release artifact for your Pi architecture if you are not on arm64.
...
When `streaming` is enabled for a camera, the app starts a local MediaMTX instance automatically and publishes video-only H.264 streams over RTSP.

Snapshot uploads for streaming-enabled cameras are read back from the local RTSP stream instead of reopening the camera device directly. This avoids device contention between the live publisher and the snapshot worker.
...
If you choose a privileged port (below 1024, for example 554), the process must run with permission to bind that port (typically root or CAP_NET_BIND_SERVICE).
...
- `mediamtx.log` — MediaMTX stdout/stderr log
- `mediamtx.pid` — PID of the app-owned MediaMTX instance

If another MediaMTX instance is already listening on `rtsp_port`, the app reuses it instead of starting its own copy.

The RTSP endpoint may not be readable immediately at process start. The service waits for the stream to become readable before starting scheduled snapshots and logs `RTSP stream is ready for snapshots` when that handoff is complete.
